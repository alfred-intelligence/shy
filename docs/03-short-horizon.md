...
git add -A
git commit -m "Initial commit: empty scaffold"
...
before:
  hooks:
    - go mod tidy
    - go run ./cli/cmd gen-man /tmp/shy-man   # generated by cobra
...
**Implementation:** `cli/internal/cmd/install.go` with helpers in
`cli/internal/install/`.
...
`cli/internal/cmd/`, add unit tests, integration-test on author's
